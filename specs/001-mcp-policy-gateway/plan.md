...
`DecisionProvider`, SQLite for local audit persistence, and a keyless fake-server
test and benchmark path. `ALLOW` forwards the original name and arguments;
...
`@modelcontextprotocol/client@2.0.0`, `@typesafe-ai/sdk@0.6.0`, Zod 4, Ajv,
...
| Provider abstraction | Jev and mock implementations depend only on the focused `DecisionProvider` contract; TypeSafe details do not enter routing or policy. | PASS | PASS |
| Keyless verification | Mock provider, fake MCP server, in-memory/temp SQLite, and subprocess stdio tests require no TypeSafe key or external service. | PASS | PASS |
